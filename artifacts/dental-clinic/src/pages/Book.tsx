...
export default function Book() {
  ...
  const preselectedServiceId = searchParams.get("service") ? parseInt(searchParams.get("service")!) : null;
  ...
  const [formData, setFormData] = useState({
    serviceId: preselectedServiceId || 0,
                ...
                {loadingServices ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {services?.map(service => (
                      <button
                        key={service.id}
                        onClick={() => setFormData(p => ({ ...p, serviceId: service.id }))}
                      ...
                      >
                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${formData.serviceId === service.id ? "bg-primary text-primary-foreground" : "bg-muted text-foreground"}`}>
              ...
              >
                ...
                <div className="grid md:grid-cols-2 gap-8">
                  <div className="border rounded-2xl p-4 bg-muted/10">
                    <Label className="mb-4 block text-lg font-bold">{t("book.step3.date")}</Label>
                    <Calendar
                      mode="single"
